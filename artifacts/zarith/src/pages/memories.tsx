          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredMemories.map((memory) => (
              ...
              >
                ...
                <p className="font-mono text-sm leading-relaxed text-[var(--text-primary)]">{memory.content}</p>

                <div className="flex flex-wrap gap-2">
                  {memory.tags.map((tag) => (
                    <span
                      key={tag}
                      className="flex items-center gap-1 px-2 py-1 bg-[var(--bg-primary)] border border-[var(--border-glow)] rounded-lg text-[10px] font-bold text-[var(--accent-cyan)]"
                    >
                      <Tag size={10} />
                      {tag.toUpperCase()}
                    </span>
